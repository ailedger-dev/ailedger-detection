...
rather than a looser one, because <standard>"). Each warrant carries a content
digest (`warrant_digest`) over its substantive fields — deterministic across
wall-clock time, so a verifier can re-run a detection and confirm the warrant's
content. This is the auditable spine the Grafana LARP monitor consumes. See
[LARP warrant](#larp-warrant) below.
