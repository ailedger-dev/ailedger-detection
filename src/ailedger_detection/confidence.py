...
- **Minimum stratum size** for inference is `min_group_size` (default 2): a group
  ...
  The default is >1 deliberately (F6): a single A/B pair in a stratum must not
  drive an overall flag — that is the small-sample false-positive this gate
  exists to suppress.
    ...
    *,
    ...
    Args:
        ...
        min_group_size: Minimum per-group event count for a stratum to evaluate
            that group (default 2). Groups below this are excluded from the
            stratum's ratio, so a trivially small stratum cannot manufacture a
            flag (F6).
