...
Every primitive result memorializes itself as a LARP warrant via `to_warrant()`
(see warrant.py): the result is a 1-cell, the evidence + rejected thresholds are
the 2-cell. The DetectionEngine (engine.py) orchestrates primitives over a
cohort and returns a warranted, digest-chained DetectionRun — the auditable
spine the Interchange and the Grafana LARP monitor consume.
