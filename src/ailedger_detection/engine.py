...
and returns a `DetectionRun`: a bundle of warrants (one per primitive) with
a chaining digest. Every run is therefore self-describing and auditable — the
audit spine the Grafana monitor consumes.
...
class DetectionRun:
    ...
    created_at: str
    run_digest: str
    """SHA-256 over `prev_digest` + the ordered warrant digests. Including the
    prior run's digest links run N to run N-1, so a sequence of runs forms an
    append-only chain (not just a within-run bundle)."""

    prev_digest: str | None = None
    """The `run_digest` of the immediately preceding run, or None for the first
    run in a chain. Recorded so the chain is reconstructible and verifiable."""
...
class DetectionEngine:
    ...
    @staticmethod
    def _run_digest(warrants: tuple[Warrant, ...], prev_digest: str | None) -> str:
        payload = json.dumps(
            {"prev": prev_digest, "warrants": [w.warrant_digest for w in warrants]},
            separators=(",", ":"),
        )
        ...
        *,
        ...
        Args:
            ...
            created_at: RFC3339 UTC timestamp stamped on every warrant and the
                ...
                replay/tests.
            prev_digest: The `run_digest` of the preceding run, to chain this run
                onto it (F4). None for the first run in a chain. The prior digest
                is folded into this run's digest so the chain is tamper-evident
        ...
        return DetectionRun(
            ...
            run_digest=self._run_digest(warrants, prev_digest),
