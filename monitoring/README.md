...
A Grafana dashboard for making **decision accountability** visible over AILedger
Decision Events: are decisions warranted, is the integrity chain unbroken, and
what is the warrant-rejection rate.

The dashboard is itself an accountability artifact — accountability you can see.

## Dashboard

`dashboards/ailedger-detection.json` — **AILedger — Detection** (`uid: ailedger-detection`).

Panels:

- **Warrant coverage** / **Un-warranted decisions** — share of Decision Events
  that carry a warrant.
- **Warrant-rejection rate** — rate (and trend) of waved-off decisions.
- **Integrity chain** — hash-chain verification status, per tenant.
...
2. **Dashboards → New → Import** and upload `dashboards/ailedger-detection.json`.
